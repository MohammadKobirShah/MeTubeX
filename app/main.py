...
class Config:
    _DEFAULTS = {
        ...
        'SUBTITLE_LANGUAGE_PRIORITY': 'en,bn,hi',
        'AUDIO_LANGUAGE_PRIORITY': 'bn,hi,en',
    ...
    _BOOLEAN = ('DOWNLOAD_DIRS_INDEXABLE', 'CUSTOM_DIRS', 'CREATE_CUSTOM_DIRS', 'DELETE_FILE_ON_TRASHCAN', 'HTTPS', 'ENABLE_ACCESSLOG', 'ALLOW_YTDL_OPTIONS_OVERRIDES', 'ENABLE_API_AUTH', 'ENABLE_RATE_LIMIT')
...
app = web.Application()
# Attach optional middlewares based on configuration
try:
    from app.middleware import create_auth_middleware, create_rate_limit_middleware
    from app.middleware import create_version_middleware
    # Version middleware should run early
    app.middlewares.append(create_version_middleware(config))
    if getattr(config, 'ENABLE_API_AUTH', False):
        app.middlewares.append(create_auth_middleware(config))
    if getattr(config, 'ENABLE_RATE_LIMIT', False):
        app.middlewares.append(create_rate_limit_middleware(config))
except Exception:
    # If middleware import fails, continue without hard failure
    pass
...
def parse_download_options(post: dict) -> dict:
    ...
    if subtitle_language is None:
        # Default subtitle language uses configured priority list (first entry)
        try:
            priority = str(config.SUBTITLE_LANGUAGE_PRIORITY or '').split(',')
            subtitle_language = (priority[0].strip() if priority and priority[0].strip() else 'en')
        except Exception:
...
try:
    ...
    api_v1.register_routes(app, config, dqueue, submgr, serializer, sio, parse_download_options)
except Exception as e:
    log.warning(f'Failed to register api_v1 routes: {e}')
try:
